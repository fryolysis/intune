...
def from_pitch_set(pitch_set, length):
    '''
    - generates a random `Score` that contain only the specified pitch classes
    ...
    for _ in range(length):
        n = Note(start=clock, semitones=random.choice(pitch_set))
...
def no_overlap(length):
    ...
    clock = 0
    for _ in range(length):
        n = Note(start=clock, semitones=random.randint(0,127))
        clock += random.random()
        n.halt(clock)
        notes.append(n)
        clock += random.random()
